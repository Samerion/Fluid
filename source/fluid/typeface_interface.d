module fluid.typeface_interface;
...
Typeface loadTypeface(float fontSize) {
    return loadDefaultTypeface(fontSize);
}
...
interface Typeface {
    ...
    alias defaultTypeface = .defaultTypeface;
    ...
    protected final void drawTab(ref Vector2 penPosition) const {
        ...
        penPosition.x += tabWidth(penPosition.x);

    }

    final float tabWidth(float xoffset) const {
